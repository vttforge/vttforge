...
- 9b091fc: `ContextMenuEntry.onClick` was typed `(target: HTMLElement) => void` and Foundry
...
- 7f2a3e2: `ContextMenuEntry` described the v13 shape only, so a package following it
...
- e781741: Four members Foundry has and this package did not describe. Each was found by a
...
- d08cd15: `GameSettingsApi` gained `menus`, so a hand-written stand-in for that interface
...
- 762ce7c: Building a `FoundryConfig` or `DocumentConfig` object literal now needs a document class that carries `create` and `createDocuments`. A bare `class {}` no longer satisfies it. Assignment is unchanged: `CONFIG.Actor.documentClass = MyActor` still takes any class, because a system subclass rarely redeclares those statics.
...
- dbd991a: `FoundryNamespace` types the `foundry` global, so a project stops writing its
  own.
...
- 547be4c: `game.settings.settings` is typed. It is the map of every setting the world
...
- 594195c: `TokenObjectLike` now extends a new `PlaceableObjectLike`, which adds a required `id`. A hand-written stub that stood in for a token has to grow that field.
