...
Initial functional release (v0.1 MVP slice). Foundry v13+ system runtime helpers, designed around the `/Foundry system guidance` and `/Foundry module guidance` API contracts.
...
- `BaseTypeDataModel()` — mixin over `foundry.abstract.TypeDataModel` providing a safe default `migrateData` that delegates to `super` (Foundry system guidance pitfall #8), a stub `_addDataFieldMigrations`, and a no-op `prepareDerivedData`.
- `BaseActorSheet()` — mixin over `HandlebarsApplicationMixin(foundry.applications.sheets.ActorSheetV2)` with `DEFAULT_OPTIONS` that ship the `vttforge` marker class (Foundry system guidance "Styling" rule #2).
