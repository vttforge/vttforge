...
- 9b091fc: The `system-ts` and `module-ts` templates pin `@vttforge/types` to `^0.11.0`.
...
- e781741: The `system-ts` and `module-ts` templates pin `@vttforge/types` to `^0.9.0`. On
...
- d08cd15: The `system-ts` and `module-ts` templates pin `@vttforge/types` to `^0.8.0`. On
  a 0.x line a caret pins the minor, so the old pin would scaffold a project that
  cannot resolve the version this release publishes.
...
- 9ae404a: Five fixes found by building a module on the SDK from scratch, outside this
  repo.
...
- fef514a: The plugin writes the `manifest` and `download` URLs into the built manifest.
  ...
  a HIGH finding.
- c34b8ca: The scaffold templates pin `@vttforge/core` at `^0.20.0`, the version the
...
- 712edb3: Point the scaffold templates at `@vttforge/core` 0.19, and use the document
...
- eb61b51: A call that used to succeed now exits 1. `vttforge init` refuses a flag it
...
- 0034209: Follow the `@vttforge/core` and `@vttforge/vite-plugin` minors in the scaffold's pins, so a new project gets the decorators and the build that lowers them.
...
- 89b7ffa: **Breaking.** Removes 23 internal exports from the package index.
...
- 7920fa2: Say which exports are supported, which are experimental, and which are internal.
...
- 7169a30: Stop scaffolding `template.json`, and add an audit rule for the damage it does.
...
- 4f81bfd: Follow the `@vttforge/core` minor in the scaffold's pin, so a new project gets the `onI18nInit` and `onSetup` hooks.
  
  On a `0.x` version a caret pins the minor, so `^0.11.0` would have kept new projects on the release before them.
- e9c184d: Follow the `@vttforge/styles` minor in the scaffold's pin.
...
- 8fef09a: Rewrite the four `init` templates against the current SDK.
...
- e55a6dc: Templates pin `@vttforge/core@^0.11.0`.
