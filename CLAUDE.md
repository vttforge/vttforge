...
Foundation work in progress. See `plans/PRD.md` for the full spec, `plans/TODO.md` for the punch list, and `plans/to-revise.md` for GitHub repo config items deferred until v0.1.
...
- CSS uses `@layer` for v13 cascade-layer compatibility. Vendored layer prefix `vttforge.*` (not top-level `system` — Foundry owns that namespace, see `plans/PRD.md §5.2`).
...
- `plans/` — internal planning docs (gitignored). PRD, TODO, LONG-HORIZON, to-revise.
...
- Error codes follow the `VTTF-NNNN` registry pattern (see `plans/PRD.md §7`). Append-only, stable across majors.
